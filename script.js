...
// Ticks every second regardless of fetch timing — this is what makes the header feel alive
setInterval(()=>{
  const el = document.getElementById('lastUpdated');
  if(!lastUpdateTime){ el.textContent = '--:--:--'; return; }
  const secs = Math.floor((Date.now() - lastUpdateTime)/1000);
  el.textContent = secs < 1 ? 'just now' : secs === 1 ? '1 sec आधी' : `${secs} sec आधी`;
}, 1000);

async function fetchGoldPrice(){
  // Price: fetch every tick (1s) — fast, direct, no proxy needed
  const fastResult = await fetch('https://api.gold-api.com/price/XAU', {cache:'no-store'})
    .then(r => r.ok ? r.json() : Promise.reject('bad status'))
    .catch(()=> null);

  let price = (fastResult && typeof fastResult.price === 'number') ? fastResult.price : null;

  // High/Low/prevClose: only refresh every ~15s (throttled proxy call, doesn't need to be per-second)
  const now = Date.now();
  if(!fetchGoldPrice._lastHL || now - fetchGoldPrice._lastHL > 15000){
    fetchGoldPrice._lastHL = now;
    fetchWithFallback('https://data-asg.goldprice.org/dbXRates/USD')
      .then(raw => { fetchGoldPrice._hlData = JSON.parse(raw).items[0]; })
      .catch(()=>{});
  }
  const hlData = fetchGoldPrice._hlData || null;

  if(price === null){
    if(hlData){
      renderPrice(hlData.xauPrice, hlData.xauClose, hlData.xauHigh || hlData.xauPrice, hlData.xauLow || hlData.xauPrice);
    } else {
      showOffline();
    }
    return;
  ...
  if(hlData){
    renderPrice(price, hlData.xauClose, hlData.xauHigh || price, hlData.xauLow || price);
  } else {
    renderPrice(price, price, price, price);
  }
}

function renderPrice(price, prevClose, high, low, provisional){
  try{
    const change = price - prevClose;
    const pct = (change/prevClose*100);

    const priceEl = document.getElementById('goldPrice');
    const wrapEl = document.getElementById('priceWrap');
    const arrowEl = document.getElementById('priceArrow');
    const prevShown = parseFloat(priceEl.textContent);
    priceEl.textContent = price.toFixed(2);
    document.getElementById('goldHigh').textContent = high.toFixed(2);
    document.getElementById('goldLow').textContent = low.toFixed(2);
    document.getElementById('goldGram').textContent = (price/31.1035).toFixed(2);

    // TradingView-style: flash background box + directional arrow on every real price tick
    if(!isNaN(prevShown) && prevShown !== price){
      const goingUp = price > prevShown;
      wrapEl.classList.remove('flash-up','flash-down');
      arrowEl.classList.remove('show-up','show-down');
      void wrapEl.offsetWidth; // restart animation
      wrapEl.classList.add(goingUp ? 'flash-up' : 'flash-down');
      arrowEl.classList.add(goingUp ? 'show-up' : 'show-down');
...
fetchGoldPrice();
setInterval(fetchGoldPrice, 1000); // refresh every 1s
