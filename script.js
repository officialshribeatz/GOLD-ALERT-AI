...
async function fetchGoldPrice(){
  try{
    ...
    try{
      // Primary: gold-api.com — CORS enabled, no key needed, works directly from mobile browsers
      ...
      price = d.price;
      prevClose = (typeof d.prevClosePrice === 'number') ? d.prevClosePrice : price;
      high = (typeof d.high === 'number') ? d.high : price;
      low = (typeof d.low === 'number') ? d.low : price;
    }catch(primaryErr){
      // Fallback: goldprice.org via proxy
      const raw = await fetchWithFallback('https://data-asg.goldprice.org/dbXRates/USD');
      const data = JSON.parse(raw);
      const item = data.items[0];
      price = item.xauPrice;
      prevClose = item.xauClose;
      high = item.xauHigh || price;
      low = item.xauLow || price;
    }
...
fetchGoldPrice();
setInterval(fetchGoldPrice, 30000); // refresh every 30s
