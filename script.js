...
async function fetchGoldPrice(){
  // Fire the fast source and render immediately when it lands — don't wait for the slower one
  const fastPromise = fetch('https://api.gold-api.com/price/XAU', {cache:'no-store'})
    .then(r => r.ok ? r.json() : Promise.reject('bad status'))
    .catch(()=> null);

  const hlPromise = fetchWithFallback('https://data-asg.goldprice.org/dbXRates/USD')
    .then(raw => JSON.parse(raw).items[0])
    .catch(()=> null);
  ...
  if(price === null){
    // fast source failed — wait for the slower one as last resort
    const hlData = await hlPromise;
    if(hlData){
  ...
  // Render immediately with price only (high/low = price as placeholder until real data arrives)
  renderPrice(price, price, price, price, true);

  // When goldprice.org resolves, refine with real high/low/prevClose
  const hlData = await hlPromise;
  if(hlData){
